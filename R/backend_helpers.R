...
safe_as_numeric <- function(x) { if (base::is.numeric(x)) return(x); base::suppressWarnings(base::as.numeric(base::as.character(x))) }

create_unique_id <- function(dataset_name, row_number) { clean_dataset_name <- base::gsub("[^A-Za-z0-9_\\-]", "_", base::as.character(dataset_name)); formatted_row_number <- base::sprintf("%05d", base::as.integer(row_number)); base::paste(clean_dataset_name, "Row", formatted_row_number, sep = "_") }

load_all_files <- function(file_input_df, config, session_log_f) {
  datasets <- base::list()
  for (i in 1:base::nrow(file_input_df)) {
    f_path <- file_input_df$datapath[i]; original_name <- file_input_df$name[i]; dataset_id <- tools::file_path_sans_ext(original_name)
    safe_name <- dataset_id; counter <- 1
    while (safe_name %in% base::names(datasets)) { safe_name <- base::paste0(dataset_id, "_", counter); counter <- counter + 1 }
    tryCatch({ datasets[[safe_name]] <- .load_standardize_single_file(f_path, safe_name, config) }, error = function(e) { error_msg <- base::sprintf("Failed to load '%s': %s", original_name, e$message); session_log_f(base::paste("ERROR:", error_msg)); shiny::showNotification(error_msg, type = "error", duration = 15) })
  }
  return(datasets)
}
...
.load_standardize_single_file <- function(file_path, dataset_name, config) {
  ...
  original_colnames <- base::colnames(raw_data); cleaned_colnames <- base::make.names(original_colnames, unique = TRUE); base::colnames(raw_data) <- cleaned_colnames
  processed_data <- raw_data |> dplyr::mutate(SourceFile = dataset_name, original_row_num = dplyr::row_number(), UniqueID = create_unique_id(dataset_name, original_row_num), .before = 1)
  map_result <- .map_columns(processed_data, original_colnames, config$column_aliases); processed_data <- .standardize_datetime_columns(processed_data, map_result$rename_map)
  ...
  return(base::list(name = dataset_name, data = tibble::as_tibble(processed_data), data_original = tibble::as_tibble(raw_data), original_colnames = original_colnames, cleaned_colnames = cleaned_colnames, rename_map = map_result$rename_map, log = base::list(initial_rows = base::nrow(raw_data), initial_cols = base::ncol(raw_data))))
}

.map_columns <- function(data, original_colnames, aliases_config) {
  rename_map <- base::list()
  for (std_key in base::names(aliases_config)) { aliases <- base::unlist(aliases_config[[std_key]]); matches <- original_colnames[base::tolower(original_colnames) %in% base::tolower(aliases)]; if (base::length(matches) > 0) { rename_map[[std_key]] <- base::make.names(matches[1], unique=TRUE) } }
...
.standardize_datetime_columns <- function(data, rename_map) {
  ...
  if ("date" %in% base::names(rename_map)) {
    ...
    if (base::length(na_indices) > 0) { raw_subset <- raw_dates[na_indices]; numeric_vals <- base::suppressWarnings(base::as.numeric(raw_subset)); valid_mask <- !base::is.na(numeric_vals) & numeric_vals > 1000; if (base::any(valid_mask)) parsed[na_indices[valid_mask]] <- base::as.POSIXct(base::as.Date(numeric_vals[valid_mask], origin = "1899-12-30")) }
    data$year <- lubridate::year(parsed); data$month <- lubridate::month(parsed); data$day <- lubridate::day(parsed); return(.clean_ymd(data))
  }
  data$year <- NA_real_; data$month <- NA_real_; data$day <- NA_real_; return(data)
}
...
run_phytoclass_analysis <- function(data_for_phyto, config, fm_matrices, rename_map = NULL) {
  ...
  tryCatch({
    ...
    .get_col_name <- function(key) { if (!base::is.null(rename_map) && key %in% base::names(rename_map)) { col <- rename_map[[key]]; if (!base::is.null(col) && col %in% base::colnames(data_for_phyto)) return(col) }; aliases <- config$column_aliases[[key]] %||% key; match <- base::intersect(aliases, base::colnames(data_for_phyto))[1]; if (!base::is.na(match)) return(match); match_clean <- base::intersect(base::make.names(aliases), base::colnames(data_for_phyto))[1]; if (!base::is.na(match_clean)) return(match_clean); return(NA) }
    
    dvchla_col <- .get_col_name("Dvchla"); use_pro <- FALSE
    if (!base::is.na(dvchla_col)) { dvchla_sum <- base::sum(safe_as_numeric(data_for_phyto[[dvchla_col]]), na.rm = TRUE); if (base::is.finite(dvchla_sum) && dvchla_sum > 1e-9) use_pro <- TRUE }
...
.build_sm_matrix_advanced <- function(data, standard_pigment_names, col_finder_func) {
  sm_df <- base::data.frame(base::matrix(0, nrow = base::nrow(data), ncol = base::length(standard_pigment_names))); base::colnames(sm_df) <- standard_pigment_names; base::rownames(sm_df) <- if("UniqueID" %in% base::colnames(data)) base::as.character(data$UniqueID) else base::as.character(base::seq_len(base::nrow(data)))
  for (std_name in standard_pigment_names) { actual_col_name <- col_finder_func(std_name); if (!base::is.na(actual_col_name) && actual_col_name %in% base::colnames(data)) { vals <- safe_as_numeric(data[[actual_col_name]]); vals[!base::is.finite(vals)] <- 0; vals[vals < 0] <- 0; sm_df[[std_name]] <- vals } }
...
update_config_with_new_aliases <- function(config, datasets_processed) {
  ...
  for(ds in datasets_processed) { if (base::is.null(ds$rename_map) || base::length(ds$rename_map) == 0) next; for(key in base::names(ds$rename_map)) { mapped_col <- ds$rename_map[[key]]; if (base::is.null(mapped_col) || mapped_col == "") next; original_idx <- base::which(ds$cleaned_colnames == mapped_col); if (base::length(original_idx) > 0) { orig_name <- ds$original_colnames[original_idx[1]]; existing_aliases <- base::unlist(config$column_aliases[[key]]); if (!orig_name %in% existing_aliases) config$column_aliases[[key]] <- base::c(existing_aliases, orig_name) } } }
  return(config)
}
...
check_resolution_capabilities <- function(ds_obj, config, fm_matrices) {
  ...
  warnings_list <- base::character(0); data <- ds_obj$data; rename_map <- ds_obj$rename_map; use_pro <- FALSE
  if ("Dvchla" %in% base::names(rename_map)) { col_name <- rename_map[["Dvchla"]]; if (base::sum(safe_as_numeric(data[[col_name]]), na.rm = TRUE) > 0) use_pro <- TRUE }
  fm_matrix <- if(use_pro) fm_matrices$Fm_Pro else fm_matrices$Fm_NoPro; available_pigments <- base::character(0)
  for (pig in base::colnames(fm_matrix)) { user_col <- NA; if (pig %in% base::names(rename_map)) user_col <- rename_map[[pig]] else { for (key in base::names(config$column_aliases)) { if (pig %in% config$column_aliases[[key]]) { if (key %in% base::names(rename_map)) user_col <- rename_map[[key]]; break } } }; if (!base::is.na(user_col)) { if (base::sum(safe_as_numeric(data[[user_col]]), na.rm = TRUE) > 1e-9) available_pigments <- base::c(available_pigments, pig) } }
  for (cls in base::rownames(fm_matrix)) { class_ratios <- fm_matrix[cls, ]; needed <- base::names(class_ratios)[class_ratios > 0]; missing_for_class <- base::setdiff(needed, available_pigments); if (base::length(missing_for_class) > 0) { warnings_list <- base::c(warnings_list, base::paste0("Class '", cls, "' cannot be calculated (Missing: ", base::paste(missing_for_class, collapse=", "), ")")) } }
  return(warnings_list)
}

load_fm_matrices <- function(config) {
  fm_pro_path <- config$workspace$fm_pro_matrix_path %||% ""; fm_nopro_path <- config$workspace$fm_nopro_matrix_path %||% ""
  .load_single_fm <- function(file_path) { if (!base::nzchar(file_path)) return(base::list(error = "Path empty.")); if (!base::file.exists(file_path)) return(base::list(error = base::paste("File not found:", file_path))); tryCatch({ fm_df <- readxl::read_excel(file_path, sheet = 1, .name_repair = "unique"); if (base::colnames(fm_df)[1] == "...1") { base::colnames(fm_df)[1] <- "Phytoplankton_Class" }; fm_matrix <- base::as.matrix(fm_df[, -1, drop = FALSE]); base::rownames(fm_matrix) <- base::make.unique(base::as.character(fm_df[[1]])); fm_matrix[!base::is.finite(fm_matrix)] <- 0; fm_matrix[fm_matrix < 0] <- 0; return(fm_matrix) }, error = function(e) { return(base::list(error = e$message)) }) }
  Fm_Pro <- .load_single_fm(fm_pro_path); Fm_NoPro <- .load_single_fm(fm_nopro_path)
  if (base::is.list(Fm_Pro) || base::is.list(Fm_NoPro)) return(base::list(error = "Failed to load matrices. Check paths."))
